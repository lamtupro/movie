...
const MovieCard = ({ movie }: { movie: any }) => {
  ...
  // Xử lý image: string hoặc { url }
  let imgUrl = movie.image?.url || movie.image || '/no-image.jpeg';
  const srcImg =
  ...
  return (
    <Link href={`/${movie.slug}`} title={movie.name}>
      <div className="relative group overflow-hidden rounded-lg text-sm">
        ...
        <h2 className="text-white text-sm p-2 truncate">{movie.name}</h2>
      </div>
    </Link>
  );
};
...
const MovieSection = ({
  ...
  const getPagination = () => {
    ...
    for (const page of range) {
      if (lastPage !== 0 && (page as number) - lastPage > 1) pages.push('...');
      pages.push(page);
  ...
  return (
    <div className="bg-[#0F0F10] py-2">
      <div className="container mx-auto px-2 md:px-4">
        ...
        {/* Pagination */}
        {totalPages > 1 && <div className="mt-6">{renderPageNumbers()}</div>}
      </div>
    </div>
  );
};
